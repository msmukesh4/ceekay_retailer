...
class V1::Account::ProfilesController < V1::BaseController
	...
	# POST v1/account/profile/edit_profile
	def edit_profile
			...
			if !u.blank?
				...
				if token == params[:access_token]
					if u.blank?
						...
						if u.save
							respond_to do |format|
								format.json {render :json => { success: "true", reason: "User profile successfully updated"} }
							end
						else
							respond_to do |format|
								format.json {render :json => { success: "false", reason: "unable to save user details"} }
							end
						end
					end
				else
	...
	# POST v1/account/profile/update_password
	def update_password
		...
		if !params[:dse_code].blank? and !params[:old_password].blank? and !params[:access_token].blank? and !params[:new_password].blank?
			if auth(params[:dse_code],params[:old_password],params[:access_token])
				...
				if u.save
					respond_to do |format|
						format.json {render :json => { success: "true", reason: "password successfully changed"} }
					end
				else
					respond_to do |format|
